...
router.get('/coursecnc/:courseId', auth_1.authenticateToken, async (req, res) => {
    try {
        const userId = req.user?.id;
        if (!userId) {
            return res.status(401).json({ status: 'error', message: 'User not authenticated' });
...
router.get('/courses/:offeringId/attendance-history', auth_1.authenticateToken, async (req, res) => {
    try {
        const userId = req.user?.id;
        const { offeringId } = req.params;
...
router.get('/courses/:offeringId/attendance-analytics', auth_1.authenticateToken, async (req, res) => {
    try {
        const userId = req.user?.id;
        const { offeringId } = req.params;
        if (!userId) {
        ...
        const analyticsData = enrollments.map(enrollment => {
            ...
            if (!student)
                return null;
            const attendanceRecords = student.attendanceRecords.filter(record => record.attendance?.offeringId === offeringId);
...
router.get('/courses/:offeringId/test', auth_1.authenticateToken, async (req, res) => {
